...
namespace SubWindowSystem
{
    public class ListSelection
    {
        ...
        private void RenderSelection()
        {
            ...
            parentWindow.ResetLog();
            for (int i = 0; i < options.Count; i++)
            {
                string message = "";
                if (options[i].Name == spacerCode)
                {
                    parentWindow.WriteLine(" "); //Skip line for spacer
                    continue;
                }
                ...
                parentWindow.WriteLine(message, false);
            }
            parentWindow.RenderLog();
        }
    }
    public class SubWindowWithListSelection : SubWindow
    {
        ListSelection? selection;
        public SubWindowWithListSelection(Rectangle area, char borderChar, int borderSpacing) : base(area, borderChar, borderSpacing)
        {

        }
        public SubWindowWithListSelection(Rectangle area, char borderChar, int borderSpacing, ListSelection selection) : base(area, borderChar, borderSpacing)
        {
            this.selection = selection;
            this.selection.parentWindow = this;
        }
        public void AddSelectionClass(ListSelection selection)
        {
            this.selection = selection;
            this.selection.parentWindow = this;
        }
        public Action EnterSelectionLoop()
        {
            if (selection == null) return null;

            return selection.InputLoop();
        }
    }
}
